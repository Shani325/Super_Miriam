...
public:
	...
	virtual void reset() override { reset(0); };
	virtual bool update();
};
...
public:
	// @2: Write a c'tor
	LivesGraphics(Frame singleLife, int livesCount);
public:
	virtual cv::Mat getCollisionMask();
	virtual void draw(cv::Mat& canvas, cv::Point const& topLeft);
	// @2:reset: set number of lives:
	virtual void reset(int code) override;
	virtual void reset() override { reset(_livesCount); };
	virtual bool update();
};
...
public:
	// @2: Write a c'tor
	ScoresGraphics( int score,float fontScale = 2, int fontFace = 1);
public:
	virtual cv::Mat getCollisionMask();
	virtual void draw(cv::Mat& canvas, cv::Point const& topLeft);
	// @2:reset: should set current score:
	virtual void reset(int code) override;
	virtual void reset() override { reset(_score); };
	virtual bool update();
};